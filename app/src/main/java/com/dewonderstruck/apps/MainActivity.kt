        ...
        when (menuId) {
            ...
            R.id.nav_logout_login -> {
                ...
                psDialogMsg!!.okButton.setOnClickListener({ view: View? ->
                    psDialogMsg!!.cancel()
                    hideBottomNavigation()
                    userViewModel!!.deleteUserLogin(user).observe(this, Observer<Resource<Boolean?>> { status: Resource<Boolean?>? ->
                        if (status != null) {
                            this.menuId = 0
                            setToolbarText(binding!!.toolbar, getString(R.string.app__app_name))
                            isLogout = true
                ...
                when (resource.status) {
                    Status.SUCCESS ->                             // Success State
                        ...
                        if (resource.data != null) {
                            pref!!.edit().putString(Constants.SHOP_PHONE_NUMBER, resource.data.aboutPhone1).apply()
        ...
        basketViewModel!!.allBasketList.observe(this, Observer { resourse: List<Basket>? ->
            if (resourse != null) {
                ...
                if (total == 0) {
                    ...
                    basketNotificationTextView!!.setText(total.toString())
                }
            }
        })
    }
