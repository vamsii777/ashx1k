...
class VerifyEmailFragment : PSFragment(), DiffUtilDispatchedInterface {
    //region Variables
...
//        }
    ...
    override fun initData() {
        val itemList: LiveData<Resource<UserLogin>> = userViewModel!!.emailVerificationUser
        ...
        //For resent code
        userViewModel!!.resentVerifyCodeData.observe(this, Observer<Resource<Boolean>> { result: Resource<Boolean>? ->
            if (result != null) {
                when (result.status) {
                    Status.SUCCESS ->
                        //add offer text
