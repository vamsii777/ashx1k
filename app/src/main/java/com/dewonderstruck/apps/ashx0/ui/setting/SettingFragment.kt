...
class SettingFragment : PSFragment() {
    ...
    override fun initUIAndActions() {
        ...
        binding!!.get().notificationSettingTextView.setOnClickListener { view: View? -> navigationController!!.navigateToNotificationSettingActivity(requireActivity()) }
        binding!!.get().changeLanguageImageView.setOnClickListener { view: View? -> navigationController!!.navigateToNotificationSettingActivity(requireActivity()) }
    ...
    private fun logout() {
        userViewModel!!.deleteUserLogin(userViewModel!!.user).observe(this, Observer { status: Resource<Boolean?>? ->
            if (status != null) {
                ...
                hideUIForLogout()
                navigationController!!.navigateBackToProfileFragment(requireActivity())
                if (activity != null) {
    ...
    internal inner class ClearCacheAsync : AsyncTask<Void?, Void?, Boolean>() {
        protected override fun doInBackground(vararg p0: Void?): Boolean? {
            if (activity != null) {
                val glide = Glide.get(activity!!.applicationContext)
