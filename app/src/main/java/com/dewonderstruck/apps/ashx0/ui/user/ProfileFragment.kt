...
import com.dewonderstruck.apps.ashx0.ui.transaction.list.adapter.TransactionListAdapter
 ...
 * ProfileFragment
 */
class ProfileFragment : PSFragment(), DiffUtilDispatchedInterface {
    //region Variables
    ...
    private var adapter: AutoClearedValue<TransactionListAdapter>? = null
    ...
    override fun initUIAndActions() {
        ...
        binding!!.get().segmentEdit.setOnClickListener { view: View? -> navigationController.navigateToProfileEditActivity(activity) }
        ...
        binding!!.get().segFav.setOnClickListener { view: View? -> navigationController.navigateToFavouriteActivity(activity) }
        binding!!.get().segmentFavheart.setOnClickListener { view: View? -> navigationController.navigateToFavouriteActivity(activity) }
        binding!!.get().segmentSettings.setOnClickListener { view: View? -> navigationController.navigateToSettingActivity(activity) }
    ...
    override fun initAdapters() {
        val nvAdapter = TransactionListAdapter(dataBindingComponent, object : TransactionListAdapter.TransactionClickCallback {
            override fun onClick(transaction: TransactionObject) {
                navigationController.navigateToTransactionDetail(activity, transaction)
            }
        ...
        binding!!.get().transactionList.adapter = nvAdapter
    }

    override fun initData() {
        //User
        userViewModel!!.getUser(loginUserId).observe(this, Observer<Resource<User>> { listResource: Resource<User>? ->
...
//            transactionListViewModel.setTransactionListObj(String.valueOf(Config.LOAD_FROM_DB), String.valueOf(transactionListViewModel.offset), loginUserId);
        ...
        /*//Transaction
        transactionListViewModel!!.setTransactionListObj(Config.LOAD_FROM_DB.toString(), String.valueOf(transactionListViewModel!!.offset), loginUserId)
