 ...
 * GalleryFragment
 */
class GalleryFragment : PSFragment(), DiffUtilDispatchedInterface {
    //region Variables
    ...
    private fun imageClicked(image: Image) {
        navigationController!!.navigateToDetailGalleryActivity(activity!!, imageViewModel!!.imgType!!, imageViewModel!!.id!!, image.imgId)
    }

    override fun initAdapters() {
        // ViewModel need to get from ViewModelProviders
        val nvAdapter = GalleryAdapter(dataBindingComponent, GalleryAdapter.ImageClickCallback { image: Image -> imageClicked(image) }, this)
    ...
    override fun initData() {
        ...
        try {
            if (activity != null) {
                imageViewModel!!.id = activity!!.intent.getStringExtra(Constants.IMAGE_PARENT_ID)
            }
        } catch (e: Exception) {
            Utils.psErrorLog("Error in getting intent.", e)
        }
        val imageListLiveData: LiveData<Resource<List<Image?>>> = imageViewModel!!.imageListLiveData
