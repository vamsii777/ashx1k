...
abstract class NetworkBoundResource2<ResultType, RequestType> @MainThread constructor(private val appExecutors: AppExecutors) {
    ...
    private fun fetchFromNetwork(dbSource: LiveData<ResultType>) {
        ...
        result.addSource(apiResponse) { response: ApiResponse<RequestType> ->
            ...
            if (response.isSuccessful) {
                ...
                ) { newData: ResultType -> setValue(Resource.error(response.errorMessage, newData)) }
            }
        }
    }
    ...
    init {
        ...
        result.addSource(dbSource) { data: ResultType ->
            ...
            if (shouldFetch(data)) {
                ...
                result.addSource(dbSource) { newData: ResultType -> setValue(Resource.success(newData)) }
            }
        }
    }
}